...
*First, I'd like to demonstrate to you that the results of the Fourier transform do not have the correct units natively.*

The first step is to simulate a sine wave with a frequency of 4 Hz and an
amplitude of 2.5. This ensures we know what is the actual truth.

Then we run the Fourier transform and extract the amplitudes (magnitudes) from
the complex coeffivients.

Now I compute the frequencies vector in Hertz, going from $0$ to Nyquist over
$n$ steps, the number of points in the signal.

And make a stem plot of the amplitude spectrum.

The spilkes appear in the correct frequencies, but the amplitude is around
1900, way larger than than it should be, growing larger the longer the signal
is. This happens because the basis of the fourier transform is the dot product
between the signal and a complex sine wave. Thus, the longer the signal is,
the larger the dot product will grow. The result can be normalized by dividing
by the number of points, becoming an "average" similarity between the signal
and the sine wave, which now is not affected by the length of the data.
