...
axis, as well as the magnitude. The latter represents the signal amplitude for each frequency, which can be squared to obtain their powers.
...
For each iteration within the loop we take the $i^{th}$ frequency in `NAME` and use it to create a complex sine wave (`csw`). Once we have the signal we can
calculate the dot product between the original sinal and the complex wave we
just have generated.

This dot product is the reason why the normalized time needs to have the same number of points as the original signal: otherwise we would not be able to
...
To obtain the signal amplitude in the units of the original signal requires multiplying the magnitude by two [SEE OTHER VIDEO].

Now we plot the amplitude spectrum from our Fourier transform here. The time
domain looks like [MAKE PLOT], whereas the amplitude in the frequency domain looks like:
...
know is value at specific frequencies. Thus, it is more correct to use a stem plot or a bar plot to convey the results of a Fourier transform.

We can also compare these results with the output of the fast Fourier transform
FFT function: we apply the function, and extract the amplitudes from the Fourier coefficients. As we can see the result of the function and the loop-based
result overlap. This is not a rigorous proof that these procedures are accurate, but the convergent results do provide evidence that we're doing the right thing here, because I think we can all agree to trust the Numpy FFT code.

So to understand a little bit better how to interpret these coefficients.

The outputs of this Fourier transform analysis, I'm going to plot two Fourier coefficients at four hertz

and at four point five hertz. Remember that the Fourier coefficients are complex dot products.

This is just one way of representing the information.

This is the distance away from the origin.

So let me see.

So I'll go down to this code.

So here I'm just finding the two indices in the vector of frequencies that correspond to four hertz

and 4.5 hertz.

So that's going to be this coefficient and this coefficient.

I will talk more in a later video about how to construct this vector of Hertz, the vector of frequencies,

and what exactly this line of code means.

For now, you can just assume that this is a vector that converts from frequencies in indices to frequencies

in Hertz, given our sampling rate and given the length of time of the signal.

Anyway, here I extract the magnitude and the angle of the Fourier coefficients from these two

frequency indices.

And here I plot them, so let's see what this figure looks like.

OK, so here we see two Fourier coefficients. This is the complex plane -- the real axis and the imaginary

axis.

And these are complex dot products.

These are the Fourier coefficients from two frequencies.

Now, the two frequencies are not labeled.

One of these is four and one of these is four point five hertz.

I will let you guess, based on this plot, which coefficient is four and which coefficient is 4.5.

The point is that the larger the magnitude of the dot product, which here means the further away from

the origin, the more similar the signal is to the sine wave at that frequency.

So if the dot product were really close to the origin like this, then it would mean that the signal

and the sine wave at this frequency.

Are close to orthogonal, which means that the signal looks nothing like the sine wave at that frequency.

On the other hand, the exact phase relationship between the signal and the sine wave is usually not

something that we're very interested in.

That's why it's much more common to ignore the phases and only show the magnitudes, which is the distance

away from the origin.

So this dot could be over here or it could be here or it could be here, and that would not affect the

height of this point here.

Now, that's not to say that these phase angles are unimportant.

The phase values are crucial for applying the inverse Fourier transform.

And you will see this in the section on Inverse Fourier transforms.

In this video, I showed you how the discrete time Fourier transform works. It works by computing the dot

product between the signal and a series of complex sine waves at different frequencies.

There are many important details and subtleties packed into the code that I just showed you.

And you need to understand all those details in order to really understand the Fourier transform.

But in this video, I wanted to show you how the Fourier Transform is implemented

using a for loop. Explaining all the important subtleties and details piece by piece is the goal of

basically the rest of this course.
