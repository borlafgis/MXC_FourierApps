...
Formally, frequencies should be defined by going from $0$ to Nyquist. As a
shortcut, it is possible to define the frequencies by taking the same number
of samples as time points $(n)$ ranging between $0$ and the sampling rate
(both inclusive). This is valid under the Nyquist frequency and involves a lot
less typing.

The accuracy of this shortcut depends on whether the signal has an even or odd
number of points. That's why we define two frequency vectors `hz1`and `hz2`,
with lengths of $n+1$ and $n$, respectively.

The accuracy is evaluated by
I'm going to create a pure sine wave with a frequency of 15 hertz.
Then I define a time vector that goes from $0$ to sampling rate minus one
divided by the sampling rate over an odd number of points.

Now we can confirm the accuracy by computing the power spectrum and plotting
it.

[...] I'm just taking the fft of the signal, but plotting twice, once using
`hz1`, and using `hz2`.

```{python}
def plot_difference(max_time, offset, srate=1000, freq=15):
...
For an odd number data points, going from ¿one? to the
sampling rate in $n$ points gives us an accurate result (15 Hz), whereas the
result of the other is not correct.

```{python}
## Case 1: ODD number of data points, N is correct
plot_difference(1000 + 1, offset=.1);
```

With an even nuber of points these two
swap places, wit the vector with $n+1$ data points giving the correct result.

```{python}
## Case 2: EVEN number of data points, N+1 is correct
plot_difference(1000, offset=.1);
```

The frequency resolution depends on the number of time points. If we have a
longer signal (5 seconds instead of just one), we notice the distance between
both values now is very small
...
vector you can create the frequency vector using the shortcut.

Why would I do something like this?

Imagine we get some data, with some sort of problem I can't quite figure out.
Going over the "proper" procedure takes quite a bit of time and consideration,
but using this shortcut directly shows the power spectrum in just a line.
However, it is important to remember the Nyquist frequency is midway within
the plot. Frequencies below that point are accurate, whereas the rest are not
(the sampling rate is sufficient to reconstruct such high frequencies). It is
advisable set the range of the x axis to 0 to Nyquist.

```{python}
## Case 3: longer signal
plot_difference(5000, offset=.1);
```
