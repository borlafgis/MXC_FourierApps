...
The last plot was just a way to represent information contained in the fourier coefficients. Now I'm going to plot two Fourier coefficients at $4$ and $4.5$
hertz. For this, I need to find the two indices in the frequency vector that correspond to these two frequencies [SEE NEXT VIDEO]. [...] For now, you can
...
When doing this kind of analysis, usually we are not interested in the exact phase relationship between the signal and the sine wave, so generally only magnitudes are shown. However, note phase angles also important important,
