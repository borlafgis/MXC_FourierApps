...
```{python}
correct = to_amplitude(signal, shift=0)
shifted = to_amplitude(signal, shift=10)
...
The amplitudes of a real-valued signal (most of them) are split between the
positive and the negative frequencies, with each part containing half of the
signal's energy. Over the positive part frequency increases up to Nyquist
($1/2$  of the sampling rate), then frequency decreases over the negative
frequencies.
...
The cosine is the average of two complex exponentials. Then, if $\cos (k)$ is
a real-valued wave, the expression becomes:
...
opposite directions (i.e., clockwise and counter-clockwise). These are the
positive and the negative frequencies. The frequency increases over the former
until reaching Nyquist, and, if the signal is real-valued, decreases over the
negative frequencies, which mirror the positive ones.
