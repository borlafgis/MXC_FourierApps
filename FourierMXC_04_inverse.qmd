...
The forward Fourier transform uses complex sine waves to convert time domain
signals into their equivalents in the frequency domain. The algorithm generates
sine waves with different frequencies and estimates the amplitude and phase
theat would provide the best match with the original signal (Fourier
coefficients).

The inverse fourier transform is the opposite process. The complex Fourier
coefficients describe the characteristics of each frquency sine waves.
...
```{python}
plt.plot(hz, np.abs(fCoefs[:len(hz)]))
```
...
positive_abscoefs = np.abs(fCoefs[:len(hz)])

points = np.array([hz, positive_abscoefs]).T.reshape(-1, 1, 2)
...
axs.set_xlim(0, hz.max())
axs.set_ylim(0, positive_abscoefs.max())
```
