...
So far we have been thinking about frequency as the number of cycles per
unit time/space. However, we can also think about frequency based on the angle
of a point relative to the real axis (phase), and how fast it moves around the unit circle. Following this line of thought, each point of the signal can have
a distinct instantanous frequency. It is based on changes of the phase angle
over neighbouring points along the time series. In other words, the derivative
of the phase angle time series.
$$
F \left( t, f \right)
=
\left(
...
The key pportion of the formula is the subtraction of previous phase
($\phi_{t-1, f}$) from the current one ($\phi_{t, f}$)the phases from a time
point and the previous one. The subindex $f$ just indicates that you have a signal with multiple spectral components. Thus, it is possible to define an
instantaneous frequency from multiple different frequency ranges. The fraction
on the right is a normalization factor to scale the data to its intended unit
(Hertz, in this case). The numerator ($\delta$) is just the sampling rate.

### Python

This experiment uses a linear chirp, a signal whose frequency changes over
time. It will go from 7 Hertz at the start of the signal and steadily
(linearly) increase to 25. The formula for generating the signal is the same, but now `ff` is a vector containing a range of values / Hz at the start, 11 Hz
when 2 seconds have passed, and 16 Hz at the end.
...
If we plot time ($x$) respective to the signal ($y$) we see it is a sine all
the way through, which starts slow and oscillates faster and faster as time
goes on
...
If we "naively" calculate the Fourier transform over this entire signal, the
result follows the pattern we would expect, but amplitude it depicts is smaller than it was on the time domain, and difficult to interpret, a common occurrence
when dealing with non-stationarities.
...
To calculate the instantaneous frequency of the signal, we first need a time
series of the phase angle. For this, we use apply the Hilbert transform on the
signal, and then calculate the angles. If we plot them, there are sharp jump every time the angle wraps from $+\pi$ (end of the cycle) to $-\pi$ (start of the cycle).

```{python}
# compute instantaneous frequency
angles = np.angle(scipy.signal.hilbert(signal))
...
`np.unwrap` addresses this issue, adding each cycle atop of the previous one.
The result is an an ascending line going to a very high value, which is the
summation of many, many cycles.
...
To compute the instantaneous frequency it is necessary to apply the discrete derivative, the difference of the unwrapped angles (`np.diff` function), and then multiply by $\delta/2\pi$. Note that applying unwrapping is necessary,
otherwise the "jumps" on the angle would destroy the accuracy of the derivative.

```{python}
# compute instantaneous frequency
instfreq = np.diff(unwrapped) * (srate/(2*np.pi))
...
If we plot the instantaneous frequencues ($y$) vs time ($x$), we see it goes
from 7 to 25. The oscillations around the edges stem from the Hilbert transform.