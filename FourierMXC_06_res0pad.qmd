...
The sampling rate is the number of sample points per unit time acquired by the
device. The sampling rate defines entirely the temporal resolution, staying the
same regardless the length of the signal. On the other hand, frequency
resolution is defined by the sampling rate and the number of time points.
The sampling rate is fixed, setting the nyquist frequency of the frequency-
domain signal, but the number of points between 0 and Nyquist will depend on
the length of the signal

### Python

We create three signals with a sampling rate of 100 or 1000 hertz sampling rate,
and a length of 2 or 11 seconds. With these temporal parameters we create a
Morlet wavelet (a cosine multiplied by a Gaussian, important for signal
processing). Then we take the Fourier spectrum, and normalize by its maximum
amplitude (we do not really care about the units), and plot.
...
for fmt, (srate, length) in prop_pairs.items():
    
    # define sampling rate in this round
    # srate = srates[parami] # in Hz
    
    # define time
    time = np.arange(-1, length, 1/srate)
    ...
    ax[0].plot(time,signal,fmt)
    
    # plot frequency-domain signal
    ax[1].plot(hz,np.abs(signalX[0:len(hz)]), fmt, label=f"{srate=}, {length=}")
...
On the time domain the highest temporal resolution is just tied to the sampling
rate (1000 Hz, cyan). On the frequency domain, the sampling rate only controls
the Nyquist frequency (50 when `srate=100`, cyan, blue), whereas the the
highest spectral resolution is attained by the longest signal (11 seconds,
black).